...
export async function renderVideo(args: RenderArgs): Promise<string> {
  ...
  // Collect FFmpeg's own log output so we can surface real errors if exec fails.
  ...
  ff.on("log", ({ message }: { message: string }) => {
    ...
    if (ffmpegLog.length > 200) ffmpegLog.shift(); // keep only the last 200 lines
  });
  ...
  // Load the caption font. FFmpeg.wasm ships with no fonts; drawtext
  // requires an explicit TTF/OTF on its virtual filesystem.
  onProgress({ phase: "loading font", pct: 22 });
  const fontData = await fetchFile("/fonts/caption.ttf");
  await ff.writeFile("caption.ttf", fontData);
  ...
  // - drawtext: center-anchored white text reading from quote.txt, using the
  //             font we wrote to the FS above
  const filter = [
  ...
  try {
    ...
    // If ffmpeg's own exec failed, attach the tail of its log so the user
    // sees the real reason instead of just an opaque ErrnoError.
    const tail = ffmpegLog.slice(-12).join(" | ");
    const original = e instanceof Error ? e.message : String(e);
    throw new Error(`ffmpeg exec failed: ${original} :: ${tail}`);
  }

  onProgress({ phase: "finalizing", pct: 98 });

  // ff.readFile throws FS error if out.mp4 wasn't produced — that means the
  // encode silently failed inside ffmpeg. Surface the captured log instead.
  let data: Uint8Array;
  try {
    ...
    const tail = ffmpegLog.slice(-12).join(" | ");
    const original = e instanceof Error ? e.message : String(e);
    throw new Error(`output never written: ${original} :: ${tail}`);
  }
